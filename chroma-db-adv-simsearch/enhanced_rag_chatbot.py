...
def main():
    ...
    try:
        ...
        collection = create_similarity_search_collection(
            ...
            {'description': 'Enhanced RAG chatbot with OpenAI model integration'},
            client
        )
        ...
        test_response = model.invoke("Hello, can you respond to this test message?")

        content = getattr(test_response, "content", None)
        if content:
...
Response:'''
        ...
        content = getattr(generated_response, "content", None)
        if content:
...
def enhanced_rag_food_chatbot(collection):
    ...
    print("   Powered by OpenAI's GPT Model")
    print("="*70)
...
def handle_enhanced_rag_query(collection, query: str, conversation_history: List[str]):
    ...
    # Generate enhanced RAG response using OpenAI model
...
def show_enhanced_rag_help():
    ...
    print("=" * 45)
    print("🧠 This chatbot uses OpenAI gpt-4.1-nano to understand your")
